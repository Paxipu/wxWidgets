...
| `gtk4-theme-colour-probe.c` | How does a program read a theme's named colours (`@theme_bg_color` and friends) now that `gtk_style_context_lookup_color()` is deprecated with no replacement? Establishes that CSS answers, that an off-screen widget must be created *after* the provider to see it, and that an undefined name is substituted silently -- so definedness has to be read from several expressions disagreeing. Compares every answer against the deprecated call. |
