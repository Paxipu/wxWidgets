...
| `gtk4-filedialog-portal-hang.c` | `GtkFileDialog` is what GTK4 offers in place of the deprecated `GtkFileChooser` interface; #209 has already moved `wxDirDialog` onto it and #182 would move `wxFileDialog`. Does it open? (**Not on a machine with a D-Bus session bus and no portal answering on it.** It builds its `GtkFileChooserDialog` and then never shows it -- no error, no warning, no timeout, and `ShowModal()` never returns. The same widget presented directly appears at once, which rules out the display, the theme and the harness; clearing `DBUS_SESSION_BUS_ADDRESS` makes it appear immediately, which names the cause. `GTK_USE_PORTAL=0` does not help. Same shape as the printing finding in #161. Measured on wx itself as well: a `wxDirDialog` on today's head shows nothing and blocks for ever with the bus, and opens normally without it.) |
