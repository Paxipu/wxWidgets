...
| `gtk4-dropdown-deselection.c` | Can `GtkDropDown` be returned to "nothing selected" once its model is non-empty? wxChoice cannot leave the deprecated `GtkComboBox` unless it can, because `itemcontainertest.cpp` asserts `SetSelection(wxNOT_FOUND)` after `Append()`. (**Yes, in all seven states tried** -- realized or not, mapped or not, model given at construction or set afterwards, before or after selecting something. I had reported the opposite on #183 without measuring it, and that wrong claim shaped planning for a while. The real catch is smaller and different: appending the *first* item to an empty model makes GTK select it, so a port has to put its own selection back after every model change. Pinned in `build/tools/gtk4-invariants.c` so the CI checks it on the GTK it targets, not only on a newer one.) |
